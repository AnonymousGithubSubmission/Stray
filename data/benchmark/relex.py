...
class RelationInstancesReader(DatasetReader):
    ...
    _inst_counts: Dict = defaultdict(int)  # count instances per relation type
    _pairs: Set = set()  # keep track of pairs of entities
    _bag_sizes: Dict = defaultdict(int)  # count relation types per bag
    _relation_coocur: Dict = defaultdict(int)  # count relation types per bag
    _failed_mentions_count: int = 0  # count mentions with wrong formating
    _count_direct_supervised_inst: int = 0
    _count_bag_labels: Dict = defaultdict(int)
    def __init__(self, lazy = False,
        ...
        self._inst_counts: Dict[str, int] = defaultdict(int)  # count instances per relation type
    ...
    # # e_loc doomed to fail because token_distance fail to give list[tuple] because name binding traceback
    # def _tokens_distances_tester(self, tokens):
    #     e1_loc = []
