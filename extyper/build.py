...
from extyper.types import AnyType, NoneType, TypeOfAny, Instance, CallableType, TypeVarType
...
def process_graph(graph: Graph, manager: BuildManager) -> None:
    ...
    with open('./res_whole/ff.txt'+str(datetime.now()), 'w+') as f:
...
def process_project(graph: Graph, scc: List[str], manager: BuildManager) -> None:
    ...
    while unfinished_modules:
        ...
        if sum(len(x) for x in previous) == sum(len(x) for x in now):
            break
        previous = now
    print(time.time()-start)
    try:
        with open('pendulum_unfinished','a+') as f:
            f.write('\n'.join([x.node.fullname for y in previous for x in y])+'\n')
