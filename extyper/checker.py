...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def __init__(self, errors: Errors, modules: Dict[str, MypyFile], options: Options,
                 tree: MypyFile, path: str, plugin: Plugin, union_errors=None, incompatible=None, single_incompatible=None, mutable_funcs=None, infer_dependency_map=None, manager=None, added_attr=None, func_name_recorder = None, func_name_order=None) -> None:
        ...
        self.result_file = open('result/' + 'pendulum' +'_whole_', 'a+')
        self.result_file2 = open('result/' + 'pendulum' +'_orderred_', 'a+')
    ...
    def all_type(self):
        return MaybeTypes.make_union(self.candidates)
    def suggest_class(self, cld: ClassDef):
        print(cld.name)
                ...
                with self.scope.push_class(cld.info):
                    ...
                    for s in b.body:
                        ...
                        if isinstance(s, FuncDef):
                            print(s.name)
                            if self.mode == 'CPA':
                                self.suggest_function_CPA(s,True)
                            else:
                                self.suggest_function(s,True)
                        elif isinstance(s, Decorator):
                            if self.mode == 'CPA':
                                self.suggest_function_CPA(s.func,True)
                            else:
                                self.suggest_function(s.func,True)
                        elif isinstance(s, AssignmentStmt):
                            self.suggest_assignment(s, True)
    ...
    def suggest_assignment(self, s, from_class=False):
        ...
        if len(lvalues) == 1 and isinstance(lvalues[0], NameExpr):
            ...
            if s.type:
                ...
                self.overwrite_var_node(var, node)
            else:
                if node not in self.type_map:
                    self.store_type(node, MaybeTypes.make_union(self.S1()), overwrite=True)
                    self.overwrite_var_node(var, node)
    ...
    def global_solving(self):
        ...
        for var in self.type_map:
            ...
            if flag == 0 and var not in grounds:
                grounds.append(var)
        grounds = sorted(grounds, key=lambda x:get_candidates(self.type_map[x]))
        ...
        for g in grounds:
            new_worklist = []
            type_ = self.type_map[g]
            types = type_.items if isinstance(type_, MaybeTypes) else [type_]
            for typ in types:
                id_pair = (g, typ)
    ...
    def init_candidates(self, node):
        ...
        for arg in node.arguments:
            nows[arg] = list(set([(x.type, x.args) for x in now if hasattr(x, 'type')]))
        return nows

    def suggest_function_CPA(self, node, from_class = False, force=False):
        def get_candidates(x):
            if isinstance(x, MaybeTypes):
                return len(x.items)
            else:
        ...
        def add_type(x, types):
            for typ in types:
                if typ not in x:
                    x.append(x)
            return x
        # all_annot = MaybeTypes.make_union(self.candidates)
        ...
        return True
    def suggest_function(self, node, from_class = False, force=False):
        def get_candidates(x):
            if isinstance(x, MaybeTypes):
                return len(x.items)
            else:
        ...
        if node.unanalyzed_type is not None:
            return
        is_method = bool(node.info) and not node.is_static
        if node.fullname in self.func_name_order:
            # for predicting
            pass
        ...
        for arg in node.arguments:
            if self.mode == 'PTI':
                nows[arg] = [self.new_type_var(arg)]
            else:
                nows[arg] = self.S2(self.func_candidates[node], arg)# [self.new_type_var(arg)]
            self.arg_level[arg] = 1
        ...
        while True:
            all_annots = []
            for arg in nows:
                all_annots.append(MaybeTypes.make_union(nows[arg]))
            annots = []


            for i in range(len(base.arg_kinds)):
                argument = node.arguments[i]
                all_annots[i] = MaybeTypes.make_union(nows[argument])
                if i == 0 and is_method:
                    # annots.append([AnyType(TypeOfAny.from_omitted_generics)])
                    annots.append(Instance(node.info, [AnyType(TypeOfAny.from_omitted_generics)] * len(node.info.defn.type_vars)))
                else:
            ...
            #         break
            # else:
            #     updates = self.added_attr.union(self.message)
            #     success = try_to_update(updates)
            #     if not success: # not hit or unsolvable:
            #         # 这里不要defer，使得即使有一部分语义失败，也不会无限循环
            #         # if unsolvable:
            #         #     self.defer_this_node = True
            #         # assert False
            #         worklist, updates = self.global_solving()
            #         success = try_to_update(updates) # empty or cannot update will fail, this helps solve some case that quick update fail but
            #         if not success:
            #             # assert False
            #             break 
                # else:
                #     self.type_map = {k:v for k,v in temp_type_map}
                # for ground in self.message:
                #     if ground in nows:
                #         now = nows[ground]
                #         if now == self.S1():
                #             now = self.S2()
                ...
                #             now = self.S3()
                #         nows[ground] = now
                #     elif hasattr(self, "global_vars") and ground in self.global_vars:
                #         if isS1(self.type_map[ground]):
                #             self.type_map[ground] = self.makeS2Type()
                #         else:
                #             self.type_map[ground] = self.makeS3Type()
        if self.defer_this_node:
        ...
        return True
    def overwrite_var_node(self, var, node):
        self.var_node[var] = [node]

    def update_var_node(self, var, node, overwrite=True):
        if overwrite:
            self.var_node[var] = [node]
        else:
    ...
    def makeS1Type(self):
        return MaybeTypes.make_union(self.S1())
    def makeS2Type(self):
        return MaybeTypes.make_union(self.S2())
    def makeS3Type(self, t=None):
        if t:
            t = t.items[0]
            s3 = [x for x in self.S3() if is_subtype(x, t)]
            return MaybeTypes.make_union(s3)
        return MaybeTypes.make_union(self.S3())
    def makeS4Type(self, t=None):
        if t and len(t.items) == 1: # update basic case
            t = t.items[0]
            s4 = [x for x in self.S4() if is_subtype(x, t)]
            return MaybeTypes.make_union(s4)
        return MaybeTypes.make_union(self.S4())
    def new_type_var(self, arg):
        ...
        for i in range(100):
            ...
            if new_name not in bounded_name:
                ...
                # self.tv_level[tv] = 1
                return tv
    def S1(self, candidates=None, ground=None):
        if candidates:
            added = [x for x in candidates[ground] if not isinstance(x, tuple) and not isinstance(x, AnyType)]
        else:
            added = []
        return self.s1 + added  

    def S2(self, candidates=None, ground=None):

        generic_containers = []
        arg = ground
        if arg:
            t = self.new_type_var(arg)
            ...
            t = self.new_type_var(arg)
            # self.tv_level[t] = 2
            generic_containers.append(self.named_dict_with_type_var('builtins.dict', str_type, t))
        if candidates:
        ...
        return self.s2 + added + generic_containers
    ...
    def S3_for(self, t1):
        ...
        return s3_for
    def S3(self, candidates=None, ground=None):
        if candidates:
            added = [x for x in candidates[ground] if not isinstance(x, tuple)]
            radded = []
            for add in added:
            ...
            added = []
        return self.S2(candidates=candidates, ground=ground) + self.s3 + added
    ...
    def suggest_first_pass(self) -> None:
                ...
                with self.enter_partial_types(), self.binder.top_frame_context():
                    for i, d in enumerate(self.tree.defs):
                        if isinstance(d, FuncDef):
                            if self.mode == 'CPA':
                                self.suggest_function_CPA(d)
                            else:
                                self.suggest_function(d)
                        elif isinstance(d, Decorator):
                            if self.mode == 'CPA':
                                self.suggest_function_CPA(d.func)
                            else:
                                self.suggest_function(d.func)
                        elif isinstance(d, ClassDef):
                            self.suggest_class(d)
                        elif isinstance(d, AssignmentStmt):
                            self.suggest_assignment(d)
                        else:
                            self.accept(d)
        ...
        for item, typ in expanded:
            ...
            with self.binder.top_frame_context():
                ...
                for i in range(len(typ.arg_types)):
                    ...
                    self.var_node[item.arguments[i].variable] = [item.arguments[i]]
        ...
        for arg in item.arguments:
            ...
            if arg in self.type_map:
                self.check_completed_assignment(
                    ...
                    rvalue_name='default',
                    code=codes.ASSIGNMENT, lvalue=arg, lvar=arg.variable)
            else:
    ...
    def check_assignment(self, lvalue: Lvalue, rvalue: Expression, infer_lvalue_type: bool = True,
        ...
        if isinstance(lvalue, TupleExpr) or isinstance(lvalue, ListExpr):
            ...
            #         not (isinstance(lvalue_type, PartialType) and (lvalue_type.type is None or not isinstance(lvalue_type, MaybeType)))):
            ...
            if lvalue_type:
                if isinstance(lvalue_type, PartialType) and lvalue_type.type is None:
                    ...
                    rvalue_type = self.check_completed_assignment(lvalue_type, rvalue, context=rvalue,
                                                               code=codes.ASSIGNMENT, lvalue=lvalue, lnode=temp_lnode, lvar=lvar)
                ...
                if rvalue_type and infer_lvalue_type and not isinstance(lvalue_type, PartialType):
                    ...
                    if not (isinstance(lvalue, NameExpr) and lvalue.is_special_form):
                        ...
                        # if not (lvalue.is_new_def and lvalue.node.is_initialized_in_class):
                        #     if isinstance(rvalue_type, MaybeTypes):
                        #         for typ in rvalue_type.items:
                        #             self.expr_checker.update_local_infer_map(lvalue, typ,[[(rvalue, typ)]])
                        #             # if isinstance(lvalue, NameExpr):
                        #             #     self.expr_checker.update_local_infer_map(lvalue.node, typ,[[(rvalue, typ)]])
                        #     else:
                        #         self.expr_checker.update_local_infer_map(lvalue, rvalue_type,[[(rvalue, rvalue_type)]])
                        #         # if isinstance(lvalue, NameExpr):
                        #         #     self.expr_checker.update_local_infer_map(lvalue.node, rvalue_type,[[(rvalue, rvalue_type)]])
                        #     self.store_type(lvalue, rvalue_type)
                        
                        #     if isinstance(lvalue, NameExpr):
                        #         self.set_inferred_type(lvalue.node, lvalue, rvalue_type)
            elif index_lvalue:
            ...
            if inferred:
                ...
                # checkexpr改了之后,这里有可能为None
                # if isinstance(lvalue, NameExpr):
                #     leading_finder = LeadingFinder()
                #     rvalue.accept(leading_finder)
                #     rvalue_leading = leading_finder.first_name_expr()
                #     try:
                #         if rvalue_leading is not None and isinstance(rvalue_leading, NameExpr):
                #             r_identity = self.scope.top_function().fullname + ':' + str(rvalue_leading.node.line) + ':' + rvalue_leading.node.fullname
                #             l_identity = self.scope.top_function().fullname + ':' + str(lvalue.node.line) + ':' + lvalue.node.fullname
                        
                #             for k in self.expr_checker.infer_type:
                #                 vs = self.expr_checker.infer_type[k]
                #                 for v in vs:
                #                     self.update_local_infer_map((l_identity, k), vs)
                ...
                try:
                    ...
                    #     node, typ = key
                    #     if node == rvalue:
                    #         tmp_save.append((lvalue, typ, self.expr_checker.local_infer_map[key]))
                    #         #self.expr_checker.update_local_infer_map(lvalue, typ, self.expr_checker.local_infer_map[key])
                    ...
                    self.store_type(lvalue, rtype)
                    # if isinstance(lvalue, NameExpr):
                    #     self.store_type(lvalue.node, rtype)

                except Exception:
                    print('error l identity')
        ...
        for rvalue_type in rvalue_types:
            ...
            if isinstance(rvalue_type, AnyType):
                ...
                if self.type_is_iterable(rvalue_type) and isinstance(rvalue_type, Instance):
                    self.check_multi_assignment_from_iterable(lvalues, rvalue_type, rvalue, 
    ...
    def check_multi_assignment_from_tuple(self, lvalues: List[Lvalue], rvalue: Expression,
        ...
        if self.check_rvalue_count_in_assignment(lvalues, len(rvalue_type.items), context):
            ...
            if not undefined_rvalue:
                ...
                if isinstance(reinferred_rvalue_type, MaybeTypes):
                    reinferred_rvalue_type = reinferred_rvalue_type.items[0]
            ...
            for lv, rv_type in zip(left_lvs, left_rv_types):
                temp_node = self.temp_node(rv_type, context)
                # self.store_type(temp_node, rv_type)
                # self.infer_variable_type()
                self.expr_checker.add_infer_type((lv, rv_type), [(rvalue, rvalue_type)]) # suggest no maybe
                # self.check_definition(lv, temp_node, infer_lvalue_type)
                # self.check_assignment(lv, temp_node, infer_lvalue_type) # 这里不使用check assignment，因为temp node被默认为和this node一致，而这种情况不行，所以不走那个流程
                # if isinstance(rtype, MaybeTypes):
                # for typ in rtype.items:
                #     self.expr_checker.update_local_infer_map(lvalue, typ,[[(rvalue, typ)]])
                # else:
                #     self.expr_checker.update_local_infer_map(lvalue, rtype,[[(rvalue, rtype)]])
                
                self.store_type(lv, rv_type, False)
                if hasattr(lv, "node"):
                    self.var_node[lv.node] = [lv]
            if star_lv:
                # not p
                list_expr = ListExpr([self.temp_node(rv_type, context)
                                      for rv_type in star_rv_types])
                list_expr.set_line(context.get_line())
                list_type = self.expr_checker.accept(list_expr)
                if isinstance(list_type, MaybeTypes):
                    list_type = list_type.items[0]
                self.expr_checker.add_infer_type((list_expr, list_type), [(rvalue, rvalue_type)])
                self.check_assignment(star_lv.expr, list_expr, infer_lvalue_type)
            for lv, rv_type in zip(right_lvs, right_rv_types):
                temp_node = self.temp_node(rv_type, context)
                self.expr_checker.add_infer_type((temp_node, rv_type), [(rvalue, rvalue_type)])
                self.check_assignment(lv, temp_node, infer_lvalue_type)
        ...
        if self.type_is_iterable(rvalue_type) and isinstance(rvalue_type, Instance):
            ...
            for lv in lvalues:
                if isinstance(lv, StarExpr):
                    ...
                    temp_node =  self.temp_node(items_type, context)
                    self.expr_checker.add_infer_type((temp_node, items_type), [(rvalue, rvalue_type)])
                    self.check_assignment(lv.expr, temp_node,
                                          infer_lvalue_type)
                else:
                    temp_node = self.temp_node(item_type, context)
                    self.expr_checker.add_infer_type((lv, item_type), [(rvalue, rvalue_type)]) # suggest no maybe

                
                    self.store_type(lv, item_type, False)
                    if hasattr(lv, "node"):
                        self.var_node[lv.node] = [lv]

                    # self.expr_checker.add_infer_type((temp_node, item_type), [(rvalue, rvalue_type)])
                    # self.check_assignment(lv, temp_node,
                    #                       infer_lvalue_type)
        else:
    ...
    def check_lvalue(self, lvalue: Lvalue) -> Tuple[Optional[Type],
        ...
        if self.is_definition(lvalue) and (
            ...
            temp_lnode = TempNode(AnyType(0), context=lvalue)
            lvalue_type = self.expr_checker.analyze_ordinary_member_access(lvalue, True, temp_rhs = temp_lnode)
            ...
            temp_lnode = TempNode(AnyType(0), context=lvalue)
            lvalue_type = self.expr_checker.analyze_ref_expr(lvalue, lvalue=True, temp_rhs = temp_lnode)
            # self.store_type(lvalue, lvalue_type)
            var = lvalue.node
    ...
    def set_inferred_type(self, var: Var, lvalue: Lvalue, type: Type) -> None:
        ...
        if var: # and not self.current_node_deferred:
            # var.node = lvalue
            # var.type = type
            self.overwrite_var_node(var, lvalue)
            # if var in self.var_node:
    ...
    def check_completed_assignment(self, lvalue_type: Optional[Type], rvalue: Expression,
                                context: Context,
                                msg: str = message_registry.INCOMPATIBLE_TYPES_IN_ASSIGNMENT,
                                lvalue_name: str = 'variable',
                                rvalue_name: str = 'expression', *,
                                code: Optional[ErrorCode] = None, lvalue = None, lnode = None, lvar = None) -> Type:
        
        
        # var_noded assignment, lnode is the original node, if assignment, must be lnode
        rvalue_type = self.expr_checker.accept(rvalue)
        rvalue_type = get_proper_type(rvalue_type)
        if isinstance(lvalue_type, MaybeTypes):
            lvalue_types = lvalue_type.items
        else:
            lvalue_types = [lvalue_type]
        if isinstance(rvalue_type, MaybeTypes):
            rvalue_types = rvalue_type.items
        else:
            rvalue_types = [rvalue_type]
            ...
            for lvalue_type in lvalue_types:
                for rvalue_type in rvalue_types:
                    if self.check_subtype(rvalue_type, lvalue_type,context, msg):
                        ...
                        self.expr_checker.add_incompatible(lnode, lvalue_type, rvalue, rvalue_type)
            return
        if (isinstance(lvalue, Var) and lvar.is_initialized_in_class and lvalue.is_new_def) or (isinstance(lvalue, Argument)):
            ...
            for lvalue_type in lvalue_types:
                for rvalue_type in rvalue_types:
                    if self.check_subtype(rvalue_type, lvalue_type,context, msg):
                        ok = True
                        return_types.append(rvalue_type)
                    else:
            ...
            return_type = MaybeTypes.make_union(return_types)
            self.overwrite_var_node(lvar, lvalue)
            # self.store_type(lvalue, return_type, overwrite=True)
        else:
            for lvalue_type in lvalue_types:
                for rvalue_type in rvalue_types:
                    if self.check_subtype(rvalue_type, lvalue_type,context, msg):
                        self.expr_checker.get_ground_id_pair(lnode, lvalue_type)
                        self.expr_checker.add_infer_type((lvalue, rvalue_type), [(lnode, lvalue_type), (rvalue, rvalue_type)])
                        return_types.append(rvalue_type)
                        ok = True
                    else:
                        self.expr_checker.add_incompatible(lnode, lvalue_type, rvalue, rvalue_type)
            ...
            return_type = MaybeTypes.make_union(return_types)
            if not ok:
                self.expr_checker.emit_message_call(lnode, [rvalue])
            if lvar:
                if isinstance(lvalue, MemberExpr) and lvalue.expr.name == 'self' :
                    self.overwrite_var_node(lvar, lvalue)
    ...
    def check_member_assignment(self, instance_type: Type, attribute_type: Type,
        ...
        if not isinstance(attribute_type, Instance):
            ...
            rvalue_type = self.check_completed_assignment(attribute_type, rvalue, context,
                                                       code=codes.ASSIGNMENT, lnode = lnode, lvalue=lvalue,lvar=lvar)
            return rvalue_type, attribute_type, True
    ...
    def update_infer_dependency_map_with_constraint_pairs(self, identity, typ, constraint_pairs):
        ground_pairs = get_ground_pairs(self.dp_dag, self.expr_checker.local_infer_map, constraint_pairs)

        # if len(ground_pairs) == 0:
        #     self.update_infer_dependency_map((identity, typ), [])
        get_ground_pairs(self.dp_dag, self.expr_checker.local_infer_map, constraint_pairs)
    ...
    def update_infer_dependency_map_from_local(self, expr):
        ...
        for key in self.expr_checker.local_infer_map:
            ...
            if node == expr:
                flag = 1
                for possible_pairs in self.expr_checker.local_infer_map[key]:
                    self.update_infer_dependency_map_with_constraint_pairs(node, typ, possible_pairs)
        
        if flag == 0 and expr in self.type_map:
            ret = self.type_map[expr]
            rets = ret.items if isinstance(ret, MaybeTypes) else [ret]
            for r in rets:
    ...
    def visit_return_stmt(self, s: ReturnStmt) -> None:
        ...
        if s.expr is not None and not self.is_checking:
            ...
            for i, (e, b) in enumerate(zip(s.expr, s.body)):
                ...
                for var in self.var_node:
                    if var not in all_var_node:
                        ...
                        if all_var_node[var][0].line < self.var_node[var][0].line:
                            all_var_node[var] = self.var_node[var]
                self.var_node = {k:v for k,v in original_var_node.items()}
    ...
    def visit_operator_assignment_stmt(self,
        ...
        if isinstance(s.lvalue, MemberExpr):
            ...
            lvalue_type = self.expr_checker.accept(TempNode(AnyType(0), context = s.lvalue))
        ...
        if inplace:
            ...
            expr = OpExpr(s.op, temp_left, s.rvalue)
            # op -> left
            expr.set_line(s)
    ...
    def find_isinstance_check_helper(self, node: Expression) -> Tuple[TypeMap, TypeMap]:
        ...
        if is_true_literal(node):
            ...
            var_types = vartype.items if isinstance(vartype, MaybeTypes) else [vartype]
            ...
            if_type = MaybeTypes.make_union(if_types)
            else_type = MaybeTypes.make_union(else_types)

            # if_type = MaybeTypes.make_union([], from_cond=True)
            # else_type = MaybeTypes.make_union([], from_cond=True)
    ...
    def store_type_local(self, node: Expression, typ: Type, overwrite=False) -> None:
        if overwrite:
            if hasattr(self, "local_type_map"):
                self.local_type_map[node] = typ
        else:
            if node in self.type_map:
                previous_type = self.type_map[node]
                if isinstance(previous_type, MaybeTypes):
                    previous_types = previous_type.items
                    if isinstance(typ, MaybeTypes):
                        now_types = typ.items
                    else:
                        now_types = [typ]
                    all_types = previous_types + now_types
                    typ = MaybeTypes.make_union(all_types)
                else:
                    all_types = set([previous_type, typ])
                    typ = MaybeTypes.make_union(list(all_types))
            if hasattr(self, "local_type_map"):
                self.local_type_map[node] = typ
    def store_type(self, node: Expression, typ: Type, overwrite=True) -> None:
        if overwrite:
            ...
            if node in self.type_map:
                ...
                if isinstance(previous_type, MaybeTypes):
                    ...
                    if isinstance(typ, MaybeTypes):
                        ...
                        now_types = [typ]
                    all_types = [previous_type] + now_types
                    typ = MaybeTypes.make_union(list(all_types))
    ...
    def push_type_map(self, type_map: 'TypeMap', node=None, typ=None) -> None:
        if type_map is None or any(isinstance(x, UninhabitedType) for x in type_map.values()) :
            ...
            for expr, type in type_map.items():
                if isinstance(type, MaybeTypes) and len(type.items) == 0:
                    continue
...
def conditional_type_map(expr: Expression,
    ...
    if proposed_type_ranges:
        ...
        if isinstance(proposed_type, AnyType) or isinstance(proposed_type, NoneType):
            return {expr:MaybeTypes.make_union([], from_cond=True)}, {expr:MaybeTypes.make_union([], from_cond=True)}
        if isinstance(current_type, MaybeTypes):
            current_types = current_type.items
        else:
        ...
        for current_type in current_types:
            if current_type:
                ...
                and is_proper_subtype(current_type, proposed_type)):
                    ...
                    yes_map[expr].append(proposed_type)
        if len(yes_map[expr]) >= 1:
            yes_map[expr] = MaybeTypes.make_union(yes_map[expr])
        else:
            yes_map[expr] = UninhabitedType()
        if len(no_map[expr]) >= 1:
            no_map[expr] = MaybeTypes.make_union(no_map[expr])
        else:
            no_map[expr] = UninhabitedType()
        return yes_map, no_map
    else:
