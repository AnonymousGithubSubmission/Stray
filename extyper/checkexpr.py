...
def remove_dups_for_reason(now_component_reasons):
    ...
    for reason in now_component_reasons:
        ...
        if all(len(x)<=1 for x in node_types.values()):
            ...
            pass
    return reasons
def get_ground_pairs(dp_dag, infer_map, init_pairs):
        ...
        for reason in reasons:
            ...
            if reason not in infer_map:
                ...
                for stubs_for_this_reason in infer_map[reason]: # this list can infer compoent
                    ...
                    grounds_for_this_stub = get_ground_pairs(dp_dag, infer_map, stubs_for_this_reason)
    ...
    return list(set([tuple(x) for x in returns])) # [(identity, typ)]
class ExpressionChecker(ExpressionVisitor[Type]):
    ...
    def get_ground_id_pair(self, var, typ):
        ...
        id_pairs =  get_ground_pairs(self.chk.dp_dag, self.local_infer_map, id_pairs)
        # ret_pairs = []
        # for y in id_pairs:
        #     new_pair = []
        #     for x in y:
    ...
    def analyze_ref_expr(self, e: RefExpr, lvalue: bool = False, temp_rhs = None) -> Type:
        if isinstance(e, TempNode):
            ...
            # Variable reference.
                if node in self.chk.var_node:
                    def_nodes = self.chk.var_node[node]
                    real_types = []
                    for def_node in def_nodes:
                        assert def_node in self.chk.type_map, "error"
                        
                        if def_node in self.chk.type_map:
                            real_type = self.chk.type_map[def_node]
                            real_type = self.narrow_type_from_binder(e, real_type)
                            if isinstance(real_type, MaybeTypes):
                                for typ in real_type.items:
                                    self.add_infer_type((e, typ), [(def_node, typ)]) # 这里改用e.type是否更合理
                            else:
                                typ = real_type
                                self.add_infer_type((e, typ), [(def_node, typ)])
                        if isinstance(real_type, MaybeTypes):
                            real_types.extend(real_type.items)
                        else:
                            real_types.append(real_type)
                    
                    result = MaybeTypes.make_union(real_types)
                ...
                for def_node in def_nodes:
                    if def_node in self.chk.type_map:
                        ...
                        if def_node in self.chk.type_map:
                            ...
                            original_type = self.chk.type_map[def_node]
                            original_types = original_type.items if isinstance(original_type, MaybeTypes) else [original_type]
                            narrowed_type = self.narrow_type_from_binder(e, original_type)
                            if isinstance(narrowed_type, MaybeTypes):
                                real_types.extend(narrowed_type.items)
                            else:
                                real_types.append(narrowed_type)
                            if e is not def_node:
                                narrowed_types = narrowed_type.items if isinstance(narrowed_type, MaybeTypes) else [narrowed_type]
                                for narrowed_type in narrowed_types:
                                    atomic_types = set(narrowed_type.items) if isinstance(narrowed_type, UnionType) else set([narrowed_type])
                                    for ot in original_types:
                                        ots = set(ot.items) if isinstance(ot, UnionType) else set([ot])
                                        if ots.issuperset(atomic_types):
                                            self.add_infer_type((named_left, narrowed_type),[(def_node, ot)])
    ...
    def emit_message_call(self, callee, args):
        ...
        pass
    def add_infer_type(self, typ, from_ids, node_only=False, not_node=False):
        # self.add_path_constrains(from_ids)
        if any(x[0] == typ[0] for x in from_ids):
            return
    ...
    def analyze_var_ref(self, var: Var, context: Context) -> Type:
        return AnyType(0)
        if var in self.chk.var_node:# var.type:
            var_node = self.chk.var_node[var]
            var_type = get_proper_type(self.chk.type_map[var_node])
            ...
            # if isinstance(var_type, MaybeTypes):
            #     for item in var_type.items:
            #         self.add_infer_type((context, item), [(var, item)])
    ...
    def visit_call_expr_inner(self, e: CallExpr, allow_none_return: bool = False) -> Type:
        ...
        if (self.chk.options.disallow_untyped_calls and
            ...
            self.msg.untyped_function_call(callee_type, e) # T-ignored-inrev
        if (isinstance(callee_type, CallableType)
                and not callee_type.is_type_obj()
                and unnamed_function(callee_type.name)):
            self.msg.underscore_function_call(e) # T-ignored-never-inrev
        ...
        if isinstance(callee_type, MaybeTypes):
            callee_types = callee_type.items
        else:
            ...
            if len(corrects) > 0 and len(corrects[0]) > 0 and (len(corrects[0]) == 1 or isinstance(corrects[0][1], Tuple)):
                ...
                if isinstance(corrects[0][0][0], int):
                    ...
                    for args in ok_args:
                        ok = True
                        args = tuple([(x[0], x[1]) for x in args])
                        self.add_infer_type((e, ret_type), [(e.callee, callee_type)] + list(args))
                        # self.add_infer_type((e, ret_type), [(e.callee, callee_type)])
                ...
                for correct in corrects:
                    ...
                    # self.add_infer_type((e, ret), [(e.callee, callee_type)])
            if isinstance(ret_type, MaybeTypes):
                ret_types.extend(ret_type.items)
            else:
        ...
        if len(ret_types) == 1:
            ...
            ret_type = MaybeTypes.make_union(ret_types)
        ...
        if var in self.chk.var_node:
            ...
            if len(nodes) > 0:
                ...
                if node in self.chk.type_map:
                    ...
                    return types.items[0] if isinstance(types, MaybeTypes) else types
        return None 
    ...
    def try_infer_partial_value_type_from_call(
        ...
        if self.chk.current_node_deferred:
            return None
        partial_type = get_partial_instance_type(self.get_var_type(var))
        partial_node = self.get_var_node(var)
        ...
        if (typename in self.item_args and methodname in self.item_args[typename]
            ...
            item_types = item_type.items if isinstance(item_type, MaybeTypes) else [item_type]
            ...
            for item_type in item_types:
                if extyper.checker.is_valid_inferred_type(item_type):
                    ...
                    self.add_infer_type((e.callee.expr, new_type), [(partial_node, partial_type), (e.args[0], item_type)])
                self.chk.overwrite_var_node(var, e.callee.expr)
                self.chk.store_type(e.callee.expr, MaybeTypes.make_union(new_types))
                return MaybeTypes.make_union(new_types)
    ...
    def check_call_expr_with_callee_type(self,
        ...
        #     return self.check_union_call_expr(e, object_type, member, reason, infer_type)
        # elif member is not None and isinstance(object_type, MaybeTypes):
        #     return self.check_soft_call_expr(e, object_type, member, reason, infer_type)
    ...
    def check_soft_call_expr(self, e: CallExpr, object_type: UnionType, member: str, reason = None, infer_type = None) -> Type:
        ...
        for typ in object_type.relevant_items():
            ...
            ret = self.check_call_expr_with_callee_type(narrowed, e, callable_name,
                                                             item_object_type)
            res.append(ret)
            if isinstance(ret, MaybeTypes):
                res.extend(ret.items)
            else:
                res.append(ret)
        return MaybeTypes.make_union(res)
    ...
    def check_call(self,
        ...
        if isinstance(callee, CallableType):
            ...
            return make_simplified_union(rets), AnyType(TypeOfAny.special_form)
        ...
        elif isinstance(callee, MaybeTypes):
            return self.check_soft_call(callee, args, arg_kinds, arg_names, context, arg_messages, callable_name = callable_name, object_type = object_type)
            ...
            self.add_improvement_from_pair(method_node, callee)
            return self.msg.not_callable(callee, context), AnyType(TypeOfAny.from_error) # T-used 
    ...
    def check_generic_call(self,
        ...
        # formal_types = {x:[] for x in callee.variables}
        # typevarindex_2_argindex = []
        # for i, actuals in enumerate(formal_to_actual): # this formal to those actuals
        #     if not isinstance(callee.arg_types[i], TypeVarType):
        #         continue
        #     actual_types = []
        #     for actual in actuals:
        #         actual_type = arg_types[actual]
        #         if isinstance(actual_type, MaybeTypes):
        #             actual_type = set(actual_type.items)
        #         else:
        #             actual_type = set([actual_type])
        #         actual_types.append(actual_type)
        #     final_set = set.intersection(*actual_types)
        #     formal_types[callee.arg_types[i]].append(final_set)
        #     typevarindex_2_argindex.append(i)
        # final_formal_types = {}
        # for formal in formal_types:
        #     final_formal_types[formal] = set.intersection(*formal_types[formal])
        generic_indices = set()
        ...
        generic_types = [x.items if isinstance(x, MaybeTypes) else [x] for x in generic_types]
        ...
        for ga in GA:
            ...
            for ok_arg in ok_args:
                ...
                if corrects is not None:
                    corrects.append((list(zip(args, ok_arg)), new_callee.ret_type))
            for i, g in enumerate(ga):
                a = generic_args[i]
        ...
        self.generic_arg_types = original_generic_arg_types 
        if len(rets) > 0:
            return MaybeTypes.make_union(rets), callee
        else:
            return AnyType(0), callee
        final_formal_types, tv_at = self.infer_function_type_arguments(
            callee, args, arg_kinds, formal_to_actual, context, maybe = True)
        possible_formal_types = itertools.product(*final_formal_types)
        possible_callee = []

        corrects_k_tv = defaultdict(lambda:defaultdict(list))
        rejects_k_tv = defaultdict(lambda:defaultdict(list))
        for i, possible_formal_type in enumerate(possible_formal_types):
            if self.chk.is_checking and i > 1000:
                break
            new_callee = self.apply_generic_arguments(callee, possible_formal_type, context, skip_unsatisfied=True)
            if new_callee is not None and new_callee not in possible_callee:
                possible_callee.append(new_callee)
            
            for i, tv in enumerate(callee.variables):
                correct_at = tv_at[(tv.id, possible_formal_type[i])]
                for v in correct_at:
                    idx = args.index(v)
                    for t in correct_at[v]:
                        corrects_k_tv[new_callee][idx].append((v, t, 1))

                    all_types = arg_types[idx].items if isinstance(arg_types[idx], MaybeTypes) else [arg_types[idx]]
                    for t in all_types:
                        if t not in correct_at[v]:
                            rejects_k_tv[new_callee][idx].append((v, t))

        if len(possible_callee) == 0:
            self.emit_message_call(method_node, args)
        ret = self.check_overload_call(callee, args, arg_kinds, arg_names, callable_name, object_type, context, arg_messages, 
        corrects = corrects, rejects = rejects, plausible_targets = possible_callee, method_node = method_node, corrects_k_tv=None, rejects_k_tv=None)
        return ret
        # reject: [arg, arg_node]
        # do rejection 
        callee = freshen_function_type_vars(callee)
        ...
        # compute product and do return subtitution
    ...
    def check_argument_analyzed(self, func, arg_types, formal_to_actual):
        analyzed = True
        nows = self.chk.func_candidates[func]
        for i, actuals in enumerate(formal_to_actual): # this formal to those actuals
            ...
            for actual in actuals:
                actual_type = arg_types[actual]
                actual_types = actual_type.items if isinstance(actual_type, MaybeTypes) else [actual_type]
                for at in actual_types:
                    if isinstance(at, Instance):
                        ...
                        if at not in candidates and t not in candidates:
                            analyzed = False
                            self.chk.func_candidates[func][argument].append(at)
    ...
    def infer_function_type_arguments(self, callee_type: CallableType,
        ...
        if self.chk.in_checked_function():
            ...
            inferred_args, tv_at = infer_function_type_arguments(
                ...
                strict=self.chk.in_checked_function(), maybe = maybe, args=args)
            if maybe:
            ...
            if isinstance(caller_type, MaybeTypes):
                ...
                for item in caller_type.items:
                    ...
                    if not res:
                        ...
                        if item == callee_type: # mypy.sametypes.is_same_type(item, callee_type):
                ...
                item = caller_type
                res = is_subtype(caller_type, callee_type)
                rejected = context #self.try_to_get_left_identity(context)
                rejector, mutable_func, mutable_object = try_to_get_right_identity(outer_context, context, m, caller_kind)
                # context
                if not res:
                    if rejects is not None:
                        rejects.append((rejected, item))
                    ...
                    if extyper.sametypes.is_same_type(item, callee_type):
                        correct.append((context, item, 1))
                    else:
                        correct.append((context, item, 0.5))
    ...
    def check_overload_call(self,
        ...
        if len(plausible_targets) == 0:
            for arg, arg_type in zip(args, arg_types):
                if isinstance(arg_type, MaybeTypes):
                    arg_types = arg_type.items
                else:
                    arg_types = [arg_type]
                for arg_type in arg_types:
                    if rejects is None:
        ...
        if len(args) == 0:
            ...
            try:
                ...
                for k in corrects_k:
                    ...
                    if len(corrects_now) > 0 and len(corrects_now[0]) > 0:

                        if len(corrects_now[0][0]) == 3 and (len(corrects_now[0]) == 1 or isinstance(corrects_now[0][1], Tuple)):
                            
                            choices = itertools.product(*corrects_now)
                            for i, choice in enumerate(choices):
                                if self.chk.is_checking and i >= 100:
                                    break
    ...
    def check_soft_call(self,
                         callee: MaybeTypes,
                         args: List[Expression],
                         arg_kinds: List[ArgKind],
                         arg_names: Optional[Sequence[Optional[str]]],
                         context: Context,
                         arg_messages: MessageBuilder, callable_name = None, object_type = None) -> Tuple[Type, Type]:
        ...
        return (MaybeTypes.make_union(mu_type), callee)      

        # def bi_reject(callee, args, arg_types, formal_to_actual, object_node, object_type):
        #     for i, actuals in enumerate(formal_to_actual): # this formal to those actuals
        #         if len(actuals) == 0:
        #             continue
        #         assert len(actuals) == 1
        #         actual = actuals[0]
        #         actual_types = arg_types[actual]
        #         for actual_type in actual_types:
        #             formal_types = []
        #             for now_callee in callee.items:
        #                 formal_type = now_callee.arg_types[i]
        #                 formal_types.append(formal_type)
        #             if all([x != actual_type for x in formal_types]):
        #                 self.add_incompatible(args[actual], actual_type, object_node, object_type)
        
        # def find_highest_score(callee, arg_types, formal_to_actual):
        ...
        # self.msg.disable_type_names += 1
        # subtype = callee.relevant_items()[0]
        # typs, formal_to_actual = self.check_callable_call_from_soft(subtype, args, arg_kinds, context, arg_names,
        #                            arg_messages=arg_messages, callable_name = callable_name, object_type = object_type)
        # new_typs = []   
        # for typ in typs:
        #     if isinstance(typ, MaybeTypes):
        #         new_typs.append(typ.items)
        #     else:
        #         new_typs.append([typ])
        # results = []
        # object_node = self.try_to_get_left_node(context, args)
        # bi_reject(callee, args, new_typs, formal_to_actual, object_node, object_type)
        # for now_typs in itertools.product(*new_typs):
        #     type_args = list(zip(args, now_typs))
        #     func_with_highest_socre = find_highest_score(callee, now_typs, formal_to_actual)
        #     if func_with_highest_socre is None:
        #         pass
        #         # if isinstance(context, OpExpr):
        #         #     self.add_incompatible(args, now_typs, context.left, object_type)
        #         # else:
        #         #     self.add_incompatible(args, now_typs, context.callee, )
        #     else:
        #         if object_node is not None:
        #             self.add_infer_type((context.callee , func_with_highest_socre), [(object_node, object_type)] + list(args))
        #             self.add_infer_type((context, func_with_highest_socre.ret_type), [(object_node, object_type)] + type_args)
        #         else:
        #             #self.add_infer_type((method_node, callee), list(args))
        #             self.add_infer_type((context, func_with_highest_socre.ret_type), type_args)
        #         results.append(func_with_highest_socre.ret_type)
        # self.msg.disable_type_names -= 1
        # mu_type = []
        # for res in results:
        #     if res not in mu_type:
        #         mu_type.append(res)
        # return (MaybeTypes.make_union(mu_type), callee)     
    ...
    def analyze_ordinary_member_access(self, e: MemberExpr,
        ...
        if e.kind is not None:
            ...
            if isinstance(original_type, MaybeTypes):
                original_types = original_type.items
            else:
            ...
            for original_type in original_types:
                ...
                member_type = analyze_member_access(
                    ...
                    in_literal_context=self.is_literal_context(),
                    module_symbol_table=module_symbol_table, object_node=e.expr, member_expr=member_expr)
                ...
                if not isinstance(member_type, AnyType):
                    self.add_infer_type((member_expr, member_type), [(e.expr, original_type)])
                else:
                    self.add_infer_type((member_expr, member_type), [(e.expr, original_type)], node_only=True)

                
                if isinstance(member_type, MaybeTypes):
                    ret.extend(member_type.items)
                elif not isinstance(member_type, AnyType):
                    ret.append(member_type)
                    # if isinstance(member_type, AnyType):
                    #     # self.add_single_incompatible(e.expr, original_type)
                    #     pass
                    # else:
            ...
            if len(ret) == 0:
                ...
                return MaybeTypes.make_union(ret)
            # else:
            #     base = e.expr
            #     module_symbol_table = None
                ...
                #     self.msg, original_type=original_type, chk=self.chk,
                #     in_literal_context=self.is_literal_context(),
                #     module_symbol_table=module_symbol_table, object_node=e.expr)
                # if isinstance(member_type, AnyType):
                #     self.add_single_incompatible(e.expr, original_type)
                # else:
                #     self.add_infer_type((e, member_type), [(e.expr, original_type)])
    ...
    def check_method_call_by_name(self,
        ...
        # if isinstance(base_type, MaybeTypes):
        #     return self.check_soft_method_call_by_name(method, base_type,
        #                                                 args, arg_kinds,
        #                                                 context, local_errors, original_type)
        # method_node = TempNode(AnyType(0), context=context)
        # return_node = TempNode(AnyType(0), context=context)
        # # setattr(context, f'temp_method{context.depth}', method_node)
        # # setattr(context, f'temp_return{context.depth}', return_node)
        # context.set_method(method_node)
        # context.set_return(return_node)
        ...
        if isinstance(context, OpExpr) or isinstance(context, ComparisonExpr) or isinstance(context, UnaryExpr) or method in solved_methods:
            ...
            if isinstance(base_type, MaybeTypes) or isinstance(base_type, UnionType):
                base_types = base_type.items
            else:
            ...
            for base_type in base_types: 
                ...
                if method == '__getitem__' and isinstance(context, IndexExpr) and isinstance(base_type, TupleType) and self.try_getting_int_literals(context.index) is not None:
                    ...
                    e = context
                    self.add_infer_type((e, out), [(e.base, base_type), (e.index, self.chk.type_map[e.index])])
                    ret_type = out
                    ok = True
                    ...
                    if len(corrects) > 0 and len(corrects[0]) > 0 and (len(corrects[0]) == 1 or isinstance(corrects[0][1], Tuple)):
                        ...
                        if isinstance(corrects[0][0][0], int):
                            ...
                            for args_now in ok_args:
                                ...
                                ok = True
                                self.add_infer_type((return_node, ret_type), [(method_node, callee_type)] + list(args_now))
                                # self.add_infer_type((return_node, ret_type), [(method_node, callee_type)])
                        if isinstance(ret_type, MaybeTypes):
                            ret_types.extend(ret_type.items)
                        else:
            ...
            if (len(ret_types) == 0 or not ok) and has_method:
                ...
                ret_type = ret_types[0]
                callee_type = callee_types[0]
            else:
                ret_type = MaybeTypes.make_union(ret_types)
                callee_type = MaybeTypes.make_union(callee_types)
            if return_node is not None:
                return_node.type = ret_type
            return ret_type, callee_type
        else:
    ...
    def check_soft_method_call_by_name(self,
                                        method: str,
                                        base_type: MaybeTypes,
                                        args: List[Expression],
                                        arg_kinds: List[ArgKind],
                                        context: Context,
                                        local_errors: MessageBuilder,
        ...
        for typ in base_type.relevant_items():
            ...
            item, meth_item = self.check_method_call_by_name(method, typ, args, arg_kinds,
                                                             ...
                                                             original_type)
            local_errors.disable_type_names -= 1
            if isinstance(item, MaybeTypes):
                res.extend(item.items)
            else:
                res.append(item)
            meth_res.append(meth_item)
        return MaybeTypes.make_union(res), MaybeTypes.make_union(meth_res)
    ...
    def check_boolean_op(self, e: OpExpr, context: Context) -> Type:
        ...
        if left_map is None:
            ...
            return right_type
        left_types = left_type.items if isinstance(left_type, MaybeTypes) else [left_type]
        ...
        restricted_left_type = MaybeTypes.make_union(restricted_left_types)
        if isinstance(restricted_left_type, UninhabitedType):
            ...
            if isinstance(restricted_left_type, MaybeTypes):
                lefts = restricted_left_type.items
            else:
                lefts = [restricted_left_type]
            if isinstance(right_type, MaybeTypes):
                rights = right_type.items
            else:
                rights = [right_type]
            ret_types = []
            for left_ in lefts:
            ...
            return MaybeTypes.make_union(ret_types)
    ...
    def visit_index_with_type(self, left_type: Type, e: IndexExpr,
        ...
        elif isinstance(left_type, TupleType):  #  or (isinstance(left_type, MaybeTypes) and isinstance(left_type.items[0], TupleType)): #and self.chk.in_checked_function():
    ...
    def nonliteral_tuple_index_helper(self, left_type: TupleType, index: Expression, expr:IndexExpr) -> Type:
        ...
        ret = []
        index_types = index_type.items if isinstance(index_type, MaybeTypes) else [index_type]
    ...
    def visit_tuple_expr(self, e: TupleExpr) -> Type:
        ...
        return MaybeTypes.make_union(final_items)
    ...
    def visit_super_expr(self, e: SuperExpr) -> Type:
        ...
        if type_info is None:
            ...
            return AnyType(TypeOfAny.from_error)
        if isinstance(instance_type, MaybeTypes):
            instance_type = instance_type.items[0]
    ...
    def visit_list_comprehension(self, e: ListComprehension) -> Type:
        ret_type = self.check_generator_or_comprehension(
            e.generator, 'builtins.list', '<list-comprehension>')
        if isinstance(ret_type, MaybeTypes):
            ret_types = ret_type.items
        else:
            ret_types = [ret_type]
        for r in ret_types:
    ...
    def visit_conditional_expr(self, e: ConditionalExpr, allow_none_return: bool = False) -> Type:
        ...
        if isinstance(if_type, MaybeTypes):
            if_items = if_type.items
        else:
            if_items = [if_type]
        
        if isinstance(else_type, MaybeTypes):
            else_items = else_type.items
        else:
    ...
    def accept(self,
               ...
               always_allow_any: bool = False) -> Type:
        ...
        if (node, type_context) in self.chk.cache_type_map:
        ...
        # if not (isinstance(node, MemberExpr) and isinstance(typ, MaybeTypes)): # 为什么这里要去掉这一种情况？
    ...
    def narrow_type_from_binder(self, expr: Expression, known_type: Type,
        ...
        if literal(expr) >= LITERAL_TYPE:
            ...
            if restriction is None:
                return known_type
            if isinstance(restriction, MaybeTypes):
                return restriction
