...
from extyper.server.astdiff import (
    snapshot_symbol_table, compare_symbol_table_snapshots, SnapshotItem
)
from extyper.semanal_main import (
    semantic_analysis_for_scc, semantic_analysis_for_targets, core_modules
)
from extyper.server.astmerge import merge_asts
from extyper.server.aststrip import strip_target, SavedAttributes
from extyper.server.deps import get_dependencies_of_target, merge_dependencies
from extyper.server.target import trigger_to_target
from extyper.server.trigger import make_trigger, WILDCARD_TAG
from extyper.util import module_prefix, split_target
...
class FineGrainedBuildManager:
    ...
    def trigger(self, target: str, global_type_map=None, global_incompatible=None, id_pair=None) -> List[str]:
        """Trigger a specific target explicitly.

        This is intended for use by the suggestions engine.
        """
        self.manager.errors.reset()
        self.manager.union_errors.clear()
        self.manager.incompatible.clear()
        # self.
        # 值得cache
        t1 = time.time()
        # changed_modules = propagate_changes_using_dependencies(
        ...
        t2 = time.time()
        # print('propagate time', t2-t1)
        # Preserve state needed for the next update.
        # tmp = self.update(changed_modules, [])
        
        self.previous_targets_with_errors = self.manager.errors.targets()
        self.previous_messages = self.manager.errors.new_messages()[:]
        msg_set = set(self.previous_messages)
        for f in self.manager.union_errors:
            for x in self.manager.union_errors[f]:
                msg = x.file + ':' + str(x.line) + ':' + str(1+ x.column) + ': error: ' + x.message
                if msg in msg_set:
                    msg_set.remove(msg)
        if global_type_map is not None:
            
            # global_incompatible[id_pair] = self.manager.incompatible
            global_incompatible[id_pair] = []
            for id_p in self.manager.incompatible:
                global_incompatible[id_pair].append(id_p)
            compatible = {}
            for k in global_type_map:
                if len(global_type_map[k]) == 0:
                    continue
                compatible[k] = []
                for typ in global_type_map[k]:
                    compatible[k].append(typ)
            for identity, typ in self.manager.incompatible:
                if identity in compatible:
                    if typ in compatible[identity]:
                        compatible[identity].remove(typ)
            fail = 0
            for k in compatible:
                if len(compatible[k]) == 0:
                    fail = 1
            self.previous_messages = list(msg_set)
            self.previous_messages.append(str(fail))
        return self.previous_messages
        ...
        for target in current:
            if target.startswith('<'):
                ...
                if target.endswith('.__init__>'):
                    title = target.split('.__init__>')[0]
                    for idx in deps:
                        if idx.startswith(title) and idx not in processed:
                            worklist.add(idx[1:-1])
            else:
