...
from mypy.types import AnyType, NoneType, TypeOfAny, Instance, CallableType
from mypy.nodes import ClassDef, FuncDef, MypyFile, ImportBase, Import, ImportFrom, ImportAll, SymbolTable
...
class State:
    ...
    def type_suggest_first_pass(self, s1, s2, s3, user_defined) -> None:
        if self.options.semantic_analysis_only:
            return
        with self.wrap_context():
    ...
    def type_checker(self) -> TypeChecker:
        if not self._type_checker:
            ...
            self._type_checker = TypeChecker(manager.errors, manager.modules, self.options,
                                             self.tree, self.xpath, manager.plugin, manager.union_errors, manager.incompatible, manager.single_incompatible, manager.mutable_funcs, manager.infer_dependency_map, manager, manager.added_attr)
...
def process_project(graph: Graph, scc: List[str], manager: BuildManager) -> None:
    ...
    user_type_s2.append(graph[file_].type_checker().named_type('builtins.bool'))

    # user_type_s2.append(graph[file_].type_checker().named_type('numpy.ndarray'))

    user_type_s2.append(graph[file_].type_checker().named_type('builtins.float'))
    user_type_s2.append(graph[file_].type_checker().named_type('builtins.int'))
    ...
    builtins = ['Set', 'List', 'Tuple', 'Dict', 'defaultdict', 'MutableMapping', 'Iterable', 'Mapping', 'NamedTuple']
    for module in scc:
                ...
                if name not in builtins:
                    ...
                    if isinstance(node, TypeInfo):
                        if len(node.mro) <= 2 or node.is_named_tuple:
                            ...
                            any_type = AnyType(TypeOfAny.from_omitted_generics)
                            user_type = Instance(node, [any_type] * len(node.defn.type_vars))
                            user_type_s3.append(user_type)
    
    new_s2 = []
    for t in user_type_s2 + user_type_s1:
        str_type = graph[file_].type_checker().named_type('builtins.str')

        new_s2.append(graph[file_].type_checker().named_type_with_type_var('builtins.set', t))
        new_s2.append(graph[file_].type_checker().named_type_with_type_var('builtins.list', t))
        new_s2.append(graph[file_].type_checker().named_dict_with_type_var('builtins.dict', str_type, t))
    ...
    for file_ in stale:
        ...
        user_type_s2.insert(0, graph[file_].type_checker().named_dict_with_type_var('builtins.dict', str_type, str_type))
        user_type_s2.insert(0, graph[file_].type_checker().named_dict_with_type_var('builtins.dict', str_type, object_type))
        user_type_s2.insert(0, graph[file_].type_checker().named_type_with_type_var('builtins.list', str_type))
        user_type_s2.insert(0, graph[file_].type_checker().named_type_with_type_var('builtins.set', str_type))
    ...
    for id in stale:
        graph[id].type_suggest_first_pass(user_type_s1, user_type_s2, user_type_s3, user_defined)
