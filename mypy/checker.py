...
from mypy.nodes import (
    UNBOUND_IMPORTED, Argument, SymbolTable, Statement, MypyFile, Var, Expression, Lvalue, Node,
...
from mypy.checkexpr import LeadingFinder, get_ground_pairs
...
from mypy.join import is_similar_callables, join_types
...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def __init__(self, errors: Errors, modules: Dict[str, MypyFile], options: Options,
                 tree: MypyFile, path: str, plugin: Plugin, union_errors=None, incompatible=None, single_incompatible=None, mutable_funcs=None, infer_dependency_map=None, manager=None, added_attr=None) -> None:
        ...
        self.expr_checker = mypy.checkexpr.ExpressionChecker(self, self.msg, self.plugin, union_errors, incompatible, single_incompatible, self.infer_dependency_map, self.mutable_funcs, self.added_attr, self.message)
        self.tscope = Scope()
        ...
        self.result_file = open('result/' + tree.name +'_whole_' + str(datetime.now() ), 'w+')
        self.result_file2 = open('result/' + tree.name +'_orderred_' + str(datetime.now() ), 'w+')
                ...
                with self.scope.push_class(cld.info):
                    ...
                    for s in b.body:
                        ...
                        if isinstance(s, FuncDef):
                            print(s.name)
                            success = self.suggest_function(s, True)
                            # if s.fullname[:3] == 'pin':
                            #     pass
                            # else:
                            #     break
    ...
    def suggest_function(self, node, from_class = False, force=False):
        ...
        # now = self.S2()
        for arg in node.arguments:
            nows[arg] = self.S1(self.func_candidates[node], arg)
    ...
    def S3(self, candidates=None, ground=None):
        if candidates:
            ...
            added = []
        return self.S2(candidates, ground) + self.s3 + added
    ...
    def update_infer_dependency_map_with_constraint_pairs(self, identity, typ, constraint_pairs):
        ground_pairs = get_ground_pairs(self.expr_checker.local_infer_map, constraint_pairs)
