...
from mypy.subtypes import is_subtype, is_proper_subtype, is_equivalent, non_method_protocol_members, incompatible_annot
...
def get_ground_pairs(infer_map, init_pairs):
    ...
    while worklist:
        ...
        worklist.update(set([tuple(x) for x in now_component_reasons]))
    return list(set([tuple(x) for x in ret])) # [(identity, typ)]
class ExpressionChecker(ExpressionVisitor[Type]):
    ...
    def get_ground_id_pair(self, var, typ):
        ...
        id_pairs = [(var, typ)]
        id_pairs =  get_ground_pairs(self.local_infer_map, id_pairs)
        # ret_pairs = []
        # for y in id_pairs:
        #     new_pair = []
        #     for x in y:
                        ...
                        if def_node in self.chk.type_map:
                            ...
                            if isinstance(real_type, MaybeTypes):
                                for typ in real_type.items:
                                    self.update_local_infer_map(e, typ, [[(def_node, typ)]]) # 这里改用e.type是否更合理
                            else:
                                typ = real_type
                                self.update_local_infer_map(e, typ, [[(def_node, typ)]])
                        if isinstance(real_type, MaybeTypes):
                    ...
                    if def_node in self.chk.type_map:
                        ...
                        if def_node in self.chk.type_map:
                            ...
                            if e is not def_node:
                                if isinstance(real_type, MaybeTypes):
                                    for typ in real_type.items:
                                        self.update_local_infer_map(named_left, typ, [[(def_node, typ)]])
                                else:
                                    typ = real_type
                                    self.update_local_infer_map(named_left, typ, [[(def_node, typ)]])
                        if isinstance(real_type, MaybeTypes):
    ...
    def visit_call_expr_inner(self, e: CallExpr, allow_none_return: bool = False) -> Type:
        ...
        for callee_type in callee_types:
            ...
            ret_type = self.check_call_expr_with_callee_type(callee_type, e, fullname,
                                                            object_type, member, corrects = corrects)
    ...
    def check_call_expr_with_callee_type(self,
                                         ...
                                         object_type: Optional[Type],
                                         member: Optional[str] = None, reason = None, infer_type = None, corrects = None) -> Type:
        ...
        return self.check_call(callee_type, e.args, e.arg_kinds, e,
                               ...
                               object_type=object_type, corrects = corrects, method_node=e.callee)[0]
    ...
    def check_call(self,
        ...
        if isinstance(callee, CallableType):
            ...
            return self.check_overload_call(callee, args, arg_kinds, arg_names, callable_name,
                                            object_type, context, arg_messages, corrects = corrects, method_node=method_node)
        elif isinstance(callee, AnyType) or not self.chk.in_checked_function():
            return self.check_any_type_call(args, callee)
        elif isinstance(callee, UnionType):
            for c in callee.items:
                if isinstance(c, CallableType):
                    return self.check_callable_call(c, args, arg_kinds, context, arg_names,
                                                callable_node, arg_messages, callable_name,
                                                object_type, corrects=corrects, rejects = rejects, method_node=method_node)
    ...
    def check_generic_call(self,
        ...
        corrects = corrects, rejects = rejects, plausible_targets = possible_callee, method_node = method_node, corrects_k_tv=corrects_k_tv, rejects_k_tv=rejects_k_tv)
        return ret
        # reject: [arg, arg_node]
        # do rejection 
        callee = freshen_function_type_vars(callee)
        ...
        for i, actuals in enumerate(formal_to_actual): # this formal to those actuals
            ...
            for actual in actuals:
                ...
                for at in actual_types:
                    ...
                    elif not isinstance(at, AnyType) and not isinstance(at, NoneType) and not isinstance(at, PartialType) and not isinstance(at, UnionType) :
                        analyzed = False
                        self.chk.func_candidates[func][argument].append(at)
        return analyzed
        ...
        # Only non-abstract non-protocol class can be given where Type[...] is expected...
            ...
            if isinstance(caller_type, MaybeTypes):
                ...
                if not res:
                    ...
                    if mypy.sametypes.is_same_type(item, callee_type):
                        correct.append((context, item, 1))
                    else:
                        correct.append((context, item, 0.5))
    ...
    def analyze_ordinary_member_access(self, e: MemberExpr,
        ...
        if e.kind is not None:
            ...
            if len(ret) == 0:
                if isinstance(member_type, AnyType):
                    self.add_single_attr(e.expr,original_type, e.name)
                return AnyType(TypeOfAny.from_error)
            else:
    ...
    def visit_op_expr(self, e: OpExpr) -> Type:
        ...
        if e.op in operators.op_methods:    
            ...
            for possible_expr_typ in self.infer_type:
                self.update_local_infer_map(e, possible_expr_typ, self.infer_type[possible_expr_typ])
    ...
    def visit_comparison_expr(self, e: ComparisonExpr) -> Type:
        ...
        for left, right, operator in zip(e.operands, e.operands[1:], e.operators):
            ...
            if operator == 'in' or operator == 'not in':
                ...
                method_node = TempNode(AnyType(0), e)
    ...
    def check_method_call_by_name(self,
        ...
        if isinstance(context, OpExpr) or isinstance(context, ComparisonExpr) or method in solved_methods:
        ...
        if isinstance(restricted_left_type, UninhabitedType):
            ...
            for left_ in lefts:
                for right_ in rights:
                    ret_type = make_simplified_union([left_, right_])
                    ret_types.append(ret_type)
                    self.add_infer_type((e, ret_type), [(e.left, left_), (e.right, right_)])
    ...
    def check_lst_expr(self, items: List[Expression], fullname: str,
        ...
        constructor = CallableType(
            ...
            name=tag,
            variables=[tv])
        temp_method = TempNode(constructor, context = context)
    ...
    def visit_dict_expr(self, e: DictExpr) -> Type:
        ...
        if args or not stargs:
            ...
            constructor = CallableType(
                ...
                name='<dict>',
                variables=[kt, vt])
            method_node = TempNode(constructor, context=e)
    ...
    def check_generator_or_comprehension(self, gen: GeneratorExpr,
        ...
        with self.chk.binder.frame_context(can_skip=True, fall_through=0):
            ...
            constructor = CallableType(
                ...
                variables=[tv])
            method_node = TempNode(constructor, context=gen)
            self.chk.store_type(method_node, constructor)
            corrects = []
            # rejects = []
    ...
    def visit_conditional_expr(self, e: ConditionalExpr, allow_none_return: bool = False) -> Type:
        ...
        for if_item in if_items:
            for else_item in else_items:
                if_else_item = join.join_types(if_item, else_item)
