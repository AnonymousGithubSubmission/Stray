...
def infer_constraints_for_callable(
    ...
    for type_id in type_map:
        ...
        for t in type_set:
            ...
            corrects = []
            for cons in constraints:
                if any([mypy.subtypes.is_subtype(c.target, t) for c in cons]):
        ...
        for t in final_set:
            for cons in constraints:
                for c in cons:
                    if isinstance(t, AnyType) or (mypy.subtypes.is_subtype(c.target, t) and not isinstance(c.target, AnyType)):
